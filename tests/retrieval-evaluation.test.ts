...
test("retrieval evaluation set covers supported, guarded, and abstention paths", () => {
  ...
  assert.equal(retrievalEvaluationCases.length, 24);
  assert.equal(new Set(retrievalEvaluationCases.map((item) => item.id)).size, 24);

  const supported = retrievalEvaluationCases.filter((item) => item.category === "supported");
  assert.equal(supported.length, 16);
  ...
  assert.deepEqual(ambiguous[0].requiredSourceIds, []);
});
