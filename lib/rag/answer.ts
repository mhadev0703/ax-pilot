...
import { applyRequestEscalation, groundAnalysis, insufficientEvidence, resolveEvidence } from "./guardrails";
...
export async function answer(question: string, documents: RetrievedDocument[]) {
  ...
  return applyRequestEscalation(groundAnalysis(resolveEvidence(parsed, documents), documents), question);
}
